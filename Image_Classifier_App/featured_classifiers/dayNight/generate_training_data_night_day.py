...
class Trainer:
	...
	def exit_mainloop(self, event):
		...
		if event.keysym == 'd':
			self.labelImage(self.currentFile, 'day')
		elif event.keysym == 'n':
			self.labelImage(self.currentFile, 'night')
	...
	def labelImage(self, imageFile, label):
		...
		try:
			...
			writeToCSV('training_data/nightDay.csv', self.fieldnames, trainingDatum)
		except:
			pass
